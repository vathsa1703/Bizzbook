...
async function getStoreEconomics(x, companyId, timeframe = '1year') {
  let dateFilter = '';
  if (timeframe === '1year') {
    dateFilter = x.engine === 'postgres'
      ? `AND invoice_date >= (now() - interval '365 days')`
      : `AND invoice_date >= date('now', '-365 days')`;
  }

  // 1. AOV and Frequency
  const invoiceStats = await x.get(`
    SELECT
  ...
  // 2. Margin
  const productStats = await x.get(`
    SELECT
...
async function getConfidenceScore(x, companyId, keyword, defaultConfidence = 50) {
  const signal = await x.get(`
...
async function getCampaignPerformance(x, companyId, dateFilter = '') {
  // Fetch campaigns
  const campaigns = await x.all(`
    SELECT
