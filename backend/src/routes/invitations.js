...
publicRouter.post('/accept', async (req, res, next) => {
  try {
    ...
    const inv = await dbGet(`SELECT * FROM invitations WHERE token = ? AND status = 'pending' AND (expires_at IS NULL OR expires_at > now())`, [token]);
...
router.post('/', async (req, res, next) => {
  try {
    ...
    // dynamic day offset doesn't need to be spliced into the SQL text.
    ...
    `, [req.user.companyId, email, token, role_id, department_id, branch_id, req.user.userId, Number(expires_in_days)]);
