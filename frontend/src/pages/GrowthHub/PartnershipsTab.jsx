  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-inkA dark:text-inkA-dark">{typeFilter === 'Sponsorship' ? 'Sponsorships' : 'Partnerships'}</h2>
          <p className="text-xs text-inkB dark:text-inkB-dark">{items.length} tracked</p>
        </div>
      ...
      {loading ? <div className="flex justify-center py-8"><Loader2 size={20} className="animate-spin text-indigo-500 dark:text-indigo-400" /></div> :
       items.length === 0 ? <EmptyState icon={Handshake} title={`No ${typeFilter||'partnerships'} yet`} sub="Track potential sponsors, brand partners and strategic alliances" action="Add" onAction={() => setShowForm(true)} /> :
        <div className="space-y-3">
          {items.map(p => (
            <div key={p.id} className="bg-panel dark:bg-panel-dark rounded-2xl border border-edge dark:border-edge-dark shadow-sm p-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-inkA dark:text-inkA-dark text-sm">{p.partner_name}</span>
                    <Badge color={p.status==='Active'?'green':p.status==='Prospect'?'gray':'amber'}>{p.status}</Badge>
                  </div>
                  {p.contact_name && <p className="text-xs text-inkB dark:text-inkB-dark mt-0.5">{p.contact_name} {p.contact_email && `· ${p.contact_email}`}</p>}
                  {p.description && <p className="text-xs text-gray-400 dark:text-slate-500 mt-1 line-clamp-2">{p.description}</p>}
                  {p.value_amount && <p className="text-xs font-semibold text-green-600 dark:text-green-400 mt-1">{fmt(p.value_amount)}</p>}
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-panel dark:bg-panel-dark rounded-3xl w-full max-w-lg max-h-[80vh] overflow-y-auto">
            <div className="p-5 border-b border-edge dark:border-edge-dark flex items-center justify-between">
              <h3 className="font-bold text-inkA dark:text-inkA-dark">Add {typeFilter||'Partnership'}</h3>
            ...
            <div className="p-5 space-y-3">
              {[
                ...
                <div key={key}>
                  ...
                  {multiline
                    ? <textarea value={form[key]||''} onChange={e => setForm(f => ({...f, [key]: e.target.value}))} placeholder={placeholder} rows={2} className="w-full px-3 py-2 bg-panel2 dark:bg-panel2-dark border border-edge dark:border-edge-dark rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none" />
                    : <input type={type} value={form[key]||''} onChange={e => setForm(f => ({...f, [key]: e.target.value}))} placeholder={placeholder} className="w-full px-3 py-2 bg-panel2 dark:bg-panel2-dark border border-edge dark:border-edge-dark rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" />
                  }
                </div>
              ))}
              <div>
