  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-gray-900">Investor Pipeline</h2>
          <p className="text-xs text-gray-500">{investors.length} investors tracked</p>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        {['All', ...INV_STAGES].map(s => (
          <button key={s} onClick={() => setActiveStage(s)} className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold whitespace-nowrap transition-colors flex items-center gap-1.5 ${activeStage===s ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-gray-900 text-sm">{inv.name}</span>
                    {inv.type && <Badge color="indigo">{inv.type}</Badge>}
                  </div>
                  {inv.firm && <p className="text-xs text-gray-500 ml-4 mt-0.5">{inv.firm}</p>}
                  {inv.email && <p className="text-xs text-gray-400 ml-4">{inv.email}</p>}
                  <div className="ml-4 mt-2">
                    <Badge color={inv.status==='Closed Won'?'green':inv.status==='Closed Lost'?'red':'gray'}>{inv.status}</Badge>
                  </div>
                  {inv.notes && <p className="text-xs text-gray-500 ml-4 mt-2 line-clamp-2">{inv.notes}</p>}
                </div>
                <div className="flex gap-1">
                  <button onClick={() => openEdit(inv)} className="p-2 rounded-xl hover:bg-gray-50"><Edit2 size={13} className="text-gray-400" /></button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">{editing ? 'Edit Investor' : 'Add Investor'}</h3>
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input type={type} value={form[key]} onChange={e => setForm(f => ({...f, [key]: e.target.value}))} placeholder={placeholder}
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" />
                </div>
              ))}
              <div>
