  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-gray-900">Cap Table</h2>
          <p className="text-xs text-gray-500">{capTable.shareholders.length} shareholders · {fmt(capTable.totalInvestment)} raised</p>
        </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100">
                    {['Name','Type','Shares','Ownership','Investment'].map(h => <th key={h} className="text-left py-2 px-1 text-gray-400 font-semibold">{h}</th>)}
                    <th />
                  </tr>
                </thead>
                <tbody>
                  {capTable.shareholders.map((s, i) => (
                    <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-1 text-gray-500">{s.type}</td>
                      <td className="py-2 px-1 text-gray-700">{Number(s.shares).toLocaleString()}</td>
                      <td className="py-2 px-1">
                        <span className="font-bold text-indigo-700">{pct(s.computed_pct||s.ownership_pct)}</span>
                      </td>
                      <td className="py-2 px-1 text-gray-700">{fmt(s.investment_amount)}</td>
                      <td className="py-2 px-1">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[80vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">Add Shareholder</h3>
