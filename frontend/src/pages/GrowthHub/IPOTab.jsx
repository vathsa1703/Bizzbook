...
function ItemDocuments({ itemId }) {
  ...
  return (
    <div className="mt-1.5" onClick={e => e.stopPropagation()}>
      <button onClick={toggle} className="inline-flex items-center gap-1 text-[10px] font-semibold text-indigo-600 hover:text-indigo-700">
        <Paperclip size={10} />
        {open ? 'Hide documents' : `Documents${docs ? ` (${docs.length})` : ''}`}
      </button>
      {open && (
        <div className="mt-1.5 bg-gray-50 rounded-xl p-2.5 space-y-1.5">
          {docs === null ? (
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400"><Loader2 size={11} className="animate-spin" />Loading…</div>
          ) : docs.length === 0 ? (
            <p className="text-[10px] text-gray-400">No documents uploaded yet.</p>
          ) : docs.map(d => (
            <div key={d.id} className="flex items-center gap-2 bg-white rounded-lg px-2 py-1.5 border border-gray-100">
              <Paperclip size={10} className="text-gray-400 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-medium text-gray-700 truncate">{d.original_name || d.filename}</p>
                <p className="text-[9px] text-gray-400">{(d.created_at || '').slice(0, 10)}</p>
              </div>
              <button onClick={e => download(e, d)} className="p-1 rounded hover:bg-gray-100"><Download size={11} className="text-gray-400" /></button>
              <button onClick={e => remove(e, d.id)} className="p-1 rounded hover:bg-red-50"><Trash2 size={11} className="text-red-400" /></button>
            </div>
          ))}
          <label className="flex items-center gap-1.5 text-[10px] font-semibold text-indigo-600 cursor-pointer pt-0.5">
...
export default function IPOTab() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-black text-gray-900">IPO Readiness</h2>
        <p className="text-xs text-gray-500">{data.mandatoryComplete}/{data.mandatory} mandatory items complete</p>
      </div>
            ...
            <div className="space-y-2">
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-gray-500">Mandatory items</span>
            ...
            <div key={cat} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <button onClick={() => setOpenCat(isOpen ? null : cat)} className="w-full p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${done===items.length ? 'bg-green-100' : 'bg-gray-100'}`}>
                    {done === items.length ? <CheckCircle2 size={16} className="text-green-600" /> : <Clock size={16} className="text-gray-400" />}
                  </div>
                  <div className="text-left">
                    <p className="text-sm font-bold text-gray-900">{cat}</p>
                    <p className="text-[10px] text-gray-400">{done}/{items.length} complete</p>
                  </div>
                </div>
              ...
              {isOpen && (
                <div className="border-t border-gray-50 px-4 pb-3 space-y-2">
                  {items.map(item => (
                    <div key={item.id} className="flex items-start gap-3 py-2 hover:bg-gray-50 rounded-xl px-2 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <button onClick={() => toggle(item)} className="text-left w-full">
                          <p className={`text-xs font-medium ${item.status==='Complete' ? 'text-gray-400 line-through' : 'text-gray-700'}`}>{item.title}</p>
                          {item.is_mandatory ? <span className="text-[9px] text-red-500 font-bold">MANDATORY</span> : <span className="text-[9px] text-gray-400">Optional</span>}
                        </button>
