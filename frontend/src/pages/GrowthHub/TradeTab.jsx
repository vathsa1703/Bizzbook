...
function TradeProfileForm({ initial, onSaved }) {
  ...
  return (
    <SectionCard>
      <div className="flex items-center gap-2 mb-4"><Sparkles size={18} className="text-violet-600" /><h3 className="font-bold text-gray-900">Tell us about your import/export business</h3></div>
      <p className="text-xs text-gray-500 mb-4 leading-relaxed">We'll work out every registration, licence and filing that applies to your cross-border trade — automatically.</p>
      ...
      <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Trade Profile</label>
      <div className="grid grid-cols-2 gap-2 mb-3">
        {TRADE_TOGGLES.map(({ key, label }) => {
          const on = !!form[key];
          return (
            <button key={key} onClick={() => set(key, on ? 0 : 1)} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-semibold text-left transition-colors ${on ? 'bg-indigo-50 border-indigo-400 text-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>
      ...
      <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">IEC Number (if already registered)</label>
      <input value={form.iec_number || ''} onChange={e => set('iec_number', e.target.value)} placeholder="10-digit Import Export Code" className="w-full border rounded-xl px-3 py-2.5 text-sm mb-5 bg-gray-50" />
          ...
          <SectionCard key={g.id} className="!p-0 overflow-hidden">
            <button onClick={() => setOpenId(open ? null : g.id)} className="w-full text-left p-3 flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-800 leading-tight">{g.title}</p>
                <p className="text-[11px] text-gray-400 mt-0.5">{g.department}{g.frequency ? ` · ${g.frequency}` : ''}</p>
              </div>
              <ChevronDown size={16} className={`text-gray-400 flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
            </button>
            {open && (
              <div className="px-3 pb-3 text-xs text-gray-600 space-y-2 border-t border-gray-50 pt-2">
                {g.description && <p>{g.description}</p>}
                {g.ai_explanation && <p className="bg-violet-50 border border-violet-100 rounded-lg p-2 text-violet-900">{g.ai_explanation}</p>}
                <div className="flex flex-wrap gap-3 text-[11px] text-gray-500">
                  {g.fees && <span>Fees: {g.fees}</span>}
                  {g.processing_time && <span>Time: {g.processing_time}</span>}
                </div>
                {g.penalty_info && <p className="text-red-600">{g.penalty_info}</p>}
                {g.faq && g.faq.length > 0 && (
                  <div className="space-y-1 pt-1">
                    {g.faq.map((f, i) => <div key={i}><p className="font-semibold text-gray-700">{f.q}</p><p className="text-gray-500">{f.a}</p></div>)}
                  </div>
                )}
                {g.official_website && <a href={g.official_website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">Official Website <ExternalLink size={11} /></a>}
              </div>
            )}
          </SectionCard>
        );
  ...
  return (
    <div>
      <div className="flex gap-1.5 overflow-x-auto hide-scrollbar mb-3">
        ...
        {CATS.map(c => <button key={c} onClick={() => setCatFilter(c)} className={`px-3 py-1 rounded-full text-[11px] font-bold whitespace-nowrap capitalize ${catFilter === c ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>{c}</button>)}
      </div>
      {loading ? <div className="flex justify-center py-10"><Loader2 className="animate-spin text-gray-300" size={22} /></div>
        ...
        : (
          <SectionCard>
            {rows.map(it => {
              ...
              return (
                <button key={it.id} onClick={() => onOpen(it.id)} className="w-full text-left py-3 border-b border-gray-50 last:border-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      {dr != null && it.status !== 'completed' && <span className={`text-[10px] font-bold ${dr < 0 ? 'text-red-600' : dr <= 30 ? 'text-amber-600' : 'text-gray-400'}`}>{dr < 0 ? `${Math.abs(dr)}d overdue` : `${dr}d`}</span>}
                    </div>
                  </div>
                </button>
              );
...
function DocumentsList({ onOpen, reloadKey }) {
  ...
  return (
    <SectionCard>
      <p className="text-sm font-bold text-gray-900 mb-1">Document Checklist</p>
      {rows.map(it => {
        const missing = it.missingDocs || 0;
        return (
          <button key={it.id} onClick={() => onOpen(it.id)} className="w-full text-left flex items-center justify-between gap-2 py-3 border-b border-gray-50 last:border-0">
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-800 leading-tight">{it.title}</p>
              <p className="text-[11px] text-gray-400 mt-0.5">{it.uploaded_docs || 0}/{it.required_docs} uploaded{it.expired_docs ? ` · ${it.expired_docs} expired` : ''}</p>
            </div>
...
function Copilot() {
  ...
  return (
    <SectionCard>
      <div className="flex items-center gap-2 mb-3"><div className="w-7 h-7 rounded-full bg-indigo-600 flex items-center justify-center"><span className="text-white text-[10px] font-bold">AI</span></div><p className="text-sm font-bold text-gray-900">Trade Copilot</p></div>
      <div className="space-y-2 mb-3 max-h-80 overflow-y-auto">
        {log.length === 0 && <div className="flex flex-wrap gap-2">{suggestions.map(s => <button key={s} onClick={() => ask(s)} className="text-[11px] font-semibold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-full active:scale-95">{s}</button>)}</div>}
      ...
      <div className="flex items-center gap-2">
        <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && ask(q)} placeholder="Ask about import/export…" className="flex-1 border rounded-xl px-3 py-2.5 text-sm bg-gray-50" />
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div><h2 className="text-lg font-black text-gray-900 leading-tight">Import & Export</h2><p className="text-xs text-gray-500 font-medium mt-0.5">Trade readiness, guidelines & filings</p></div>
      </div>
              ...
              <>
                <SectionCard>
                  <div className="grid grid-cols-3 gap-3 text-center mb-4">
                    <div><div className="text-2xl font-black text-indigo-600">{overview.importReady}%</div><p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">Import Ready</p></div>
                    <div><div className="text-2xl font-black text-blue-600">{overview.exportReady}%</div><p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">Export Ready</p></div>
                    <div><div className="text-2xl font-black text-emerald-600">{overview.overallScore}%</div><p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">Overall</p></div>
                  </div>
                  <div className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center gap-2"><BadgeCheck size={16} className={overview.iec.registered ? 'text-emerald-600' : 'text-gray-400'} /><span className="text-xs font-semibold text-gray-700">IEC {overview.iec.registered ? `Registered (${overview.iec.iec_number})` : overview.iec.applicable ? 'Required — not yet registered' : 'Not applicable'}</span></div>
                  </div>
                </SectionCard>
                ...
                <SectionCard>
                  <p className="text-sm font-bold text-gray-900 mb-1">Upcoming Renewals</p>
                  {overview.upcomingRenewals.length === 0 ? <p className="text-sm text-gray-400 py-4 text-center">Nothing due in the next 60 days 🎉</p>
                    : overview.upcomingRenewals.slice(0, 6).map(d => (
                      <div key={d.id} className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
                        <button onClick={() => setOpenReq(d.id)} className="text-left min-w-0 flex-1"><p className="text-sm font-semibold text-gray-800 truncate">{d.title}</p><p className="text-[11px] text-gray-400 capitalize">{d.category} · {d.next_due_date}</p></button>
                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${d.daysRemaining < 0 ? 'bg-red-50 text-red-600' : d.daysRemaining <= 7 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>{d.daysRemaining < 0 ? `${Math.abs(d.daysRemaining)}d overdue` : `${d.daysRemaining}d left`}</span>
                      </div>
                    ))}
                </SectionCard>
                ...
                <button onClick={() => api.trade.recompute().then(load)} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-gray-50 text-gray-600 text-xs font-bold border border-gray-200 active:bg-gray-100">
