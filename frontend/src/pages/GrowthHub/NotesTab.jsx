  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-inkA dark:text-inkA-dark">Notes</h2>
          <p className="text-xs text-inkB dark:text-inkB-dark">{notes.length} notes saved</p>
        </div>
      ...
      {loading ? <div className="flex justify-center py-8"><Loader2 size={20} className="animate-spin text-indigo-500 dark:text-indigo-400" /></div> :
        ...
        <div className="space-y-3">
          {notes.map(n => (
            <div key={n.id} className="bg-panel dark:bg-panel-dark rounded-2xl border border-edge dark:border-edge-dark shadow-sm p-4">
              <div className="flex items-start justify-between gap-3">
                <p className="text-sm text-inkB dark:text-inkB-dark whitespace-pre-wrap flex-1">{n.body}</p>
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => openEdit(n)} className="p-1.5 rounded-lg hover:bg-panel2 dark:hover:bg-panel2-dark"><Edit2 size={13} className="text-gray-400 dark:text-slate-500" /></button>
                  <button onClick={() => remove(n.id)} className="p-1.5 rounded-lg hover:bg-red-50"><Trash2 size={13} className="text-red-400" /></button>
                </div>
              </div>
              <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-2">{(n.created_at || '').slice(0, 16).replace('T', ' ')}</p>
            </div>
          ))}
        </div>
      }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-panel dark:bg-panel-dark rounded-3xl w-full max-w-lg max-h-[75vh] overflow-y-auto">
            <div className="p-5 border-b border-edge dark:border-edge-dark flex items-center justify-between">
              <h3 className="font-bold text-inkA dark:text-inkA-dark">{editing ? 'Edit Note' : 'Add Note'}</h3>
              <button onClick={() => setShowForm(false)}><X size={18} className="text-gray-400 dark:text-slate-500" /></button>
            </div>
            <div className="p-5 space-y-3">
              <div>
                <label className="text-xs font-semibold text-inkB dark:text-inkB-dark mb-1 block">Note</label>
