    ...
    <div className="p-4 sm:p-6">
      ...
      {teams.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {teams.map(t => (
            <button key={t.id} onClick={() => openDetail(t)} className="text-left rounded-2xl border border-slate-800 bg-slate-900 hover:border-slate-700 p-4 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0 flex-1"><div className="text-sm font-bold text-white truncate">{t.name}</div>
                  <div className="text-[11px] text-slate-500 truncate">{t.department_name || 'No department'}</div></div>
              </div>
              {t.description && <p className="text-xs text-slate-400 mt-2 line-clamp-2">{t.description}</p>}
              <div className="mt-3 flex items-center justify-between text-[11px]">
                <span className="text-slate-500">{t.member_count || 0} members</span>
                {t.lead_name && <span className="inline-flex items-center gap-1 text-amber-400"><Crown className="w-3 h-3" />{t.lead_name}</span>}
              </div>
            </button>
          ))}
        </div>
        ...
        <Modal isOpen title={detail.team?.name || 'Team'} onClose={() => setDetail(null)} size="lg">
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-gray-500">{detail.team?.department_name || 'No department'} · {detail.members?.length || 0} members</div>
              <div className="flex gap-2">
                ...
                <button onClick={() => setDeleteTarget(detail.team)} className="inline-flex items-center gap-1 text-sm font-semibold text-brand-red"><Trash2 className="w-4 h-4" />Archive</button>
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                ...
                {(detail.members || []).map(m => (
                  <div key={m.id} className="flex items-center gap-2.5 p-2 rounded-xl bg-gray-50">
                    ...
                    <div className="min-w-0 flex-1"><div className="text-sm font-medium text-gray-900 truncate">{m.name}{m.role_in_team === 'Lead' && <span className="ml-1.5 text-[10px] text-amber-600 font-semibold">LEAD</span>}</div>
