...
export const STATUS_META = {
  ...
  cancelled: { label: 'Cancelled', dot: 'bg-slate-500', col: 'border-edge dark:border-edge-dark' },
};
export const PRIORITY_META = {
  low: { label: 'Low', cls: 'text-inkB dark:text-inkB-dark bg-slate-500/15' },
  medium: { label: 'Medium', cls: 'text-blue-600 dark:text-blue-400 dark:text-blue-400 bg-blue-500/15' },
  high: { label: 'High', cls: 'text-amber-600 dark:text-amber-400 dark:text-amber-400 bg-amber-500/15' },
  urgent: { label: 'Urgent', cls: 'text-red-600 dark:text-red-400 dark:text-red-400 bg-red-500/15' },
};
...
export function TaskCard({ task, onClick }) {
  ...
  return (
    <button onClick={() => onClick(task)} className="w-full text-left rounded-xl border border-edge dark:border-edge-dark bg-panel dark:bg-panel-dark hover:border-accent/40 p-3 space-y-2">
      ...
      {task.category && <span className="inline-block text-[10px] text-inkB dark:text-inkB-dark bg-panel2 dark:bg-panel2-dark px-1.5 py-0.5 rounded">{task.category}</span>}
      {task.checklist_total > 0 && (
        <div className="flex items-center gap-1.5"><div className="flex-1 h-1.5 bg-panel2 dark:bg-panel2-dark rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{ width: `${task.progress || 0}%` }} /></div>
          <span className="text-[10px] text-inkB dark:text-inkB-dark">{task.checklist_done}/{task.checklist_total}</span></div>
      )}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-[10px] text-inkB dark:text-inkB-dark">
          ...
          {task.due_date && <span className={`inline-flex items-center gap-0.5 ${overdue ? 'text-red-600 dark:text-red-400' : ''}`}><Calendar className="w-3 h-3" />{task.due_date.slice(5)}</span>}
        </div>
      </div>
    </button>
  );
  ...
  return (
    <Modal isOpen title="New Task" onClose={onClose} size="lg">
      <div className="p-5 space-y-3">
        ...
        <div>
          <label className="block text-xs font-semibold text-inkB dark:text-inkB-dark mb-1.5">Assign to</label>
          <div className="flex gap-1.5 mb-2">
            {['employee', 'team', 'department'].map(m => (
              <button key={m} onClick={() => setAssignMode(m)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize ${assignMode === m ? 'bg-accent text-white' : 'bg-panel2 dark:bg-panel2-dark text-inkB dark:text-inkB-dark'}`}>{m}{m !== 'employee' ? '' : 's'}</button>
            ))}
          </div>
        ...
        <div className="flex gap-2 pt-2">
          <button onClick={onClose} className="flex-1 py-2 rounded-xl text-sm font-semibold text-inkB dark:text-inkB-dark bg-panel2 dark:bg-panel2-dark">Cancel</button>
...
export function TaskDetailDrawer({ taskId, onClose, onChanged }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-canvas dark:bg-canvas-dark border-l border-edge dark:border-edge-dark h-full overflow-y-auto">
        {loading || !t ? <div className="p-6 text-inkB dark:text-inkB-dark text-sm">Loading…</div> : (
          <div className="p-5 space-y-5">
            <div className="flex items-start gap-2">
              <h2 className="flex-1 text-base font-bold text-inkA dark:text-inkA-dark">{t.title}</h2>
              <button onClick={del} className="p-1.5 text-inkB dark:text-inkB-dark hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
              <button onClick={onClose} className="p-1.5 text-inkB dark:text-inkB-dark hover:text-inkA dark:hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex items-center gap-3 text-xs text-inkB dark:text-inkB-dark flex-wrap">
            ...
            {/* Checklist */}
            <div>
              <div className="text-[10px] uppercase text-inkB dark:text-inkB-dark font-semibold mb-1.5 flex items-center gap-1"><CheckSquare className="w-3.5 h-3.5" />Checklist</div>
              <div className="space-y-1">
                {(data.checklist || []).map(it => (
                  <div key={it.id} className="flex items-center gap-2 group">
                    <button onClick={() => toggleItem(it)} className="text-inkB dark:text-inkB-dark hover:text-blue-400">{it.is_done ? <CheckSquare className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> : <Square className="w-4 h-4" />}</button>
                    <span className={`flex-1 text-sm ${it.is_done ? 'line-through text-gray-400 dark:text-slate-500' : 'text-inkB dark:text-inkB-dark'}`}>{it.title}</span>
                    <button onClick={() => delItem(it)} className="opacity-0 group-hover:opacity-100 text-gray-400 dark:text-slate-500 hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
              </div>
            ...
            {/* Attachments */}
            <div>
              <div className="text-[10px] uppercase text-inkB dark:text-inkB-dark font-semibold mb-1.5 flex items-center gap-1"><Paperclip className="w-3.5 h-3.5" />Attachments</div>
              <div className="space-y-1">
                {(data.attachments || []).map(att => (
                  <div key={att.id} className="flex items-center gap-2 text-sm">
                    <Paperclip className="w-3.5 h-3.5 text-inkB dark:text-inkB-dark" /><span className="flex-1 text-inkB dark:text-inkB-dark truncate">{att.original_name}</span>
                    <button onClick={() => download(att)} className="text-inkB dark:text-inkB-dark hover:text-blue-400"><Download className="w-3.5 h-3.5" /></button>
                    <button onClick={() => delAtt(att)} className="text-inkB dark:text-inkB-dark hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
              </div>
              <input ref={fileRef} type="file" onChange={upload} className="hidden" />
              <button onClick={() => fileRef.current?.click()} className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-300"><Plus className="w-3.5 h-3.5" />Upload file</button>
            </div>

            {/* Comments */}
            <div>
              <div className="text-[10px] uppercase text-inkB dark:text-inkB-dark font-semibold mb-1.5 flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" />Comments</div>
              <div className="space-y-2 mb-2">
                {(data.comments || []).map(c => (
                  <div key={c.id} className="bg-panel dark:bg-panel-dark rounded-lg p-2.5">
                    <div className="flex items-center gap-1.5 mb-0.5"><span className="text-xs font-semibold text-inkB dark:text-inkB-dark">{c.author_name || 'User'}</span><span className="text-[10px] text-gray-400 dark:text-slate-500">{(c.created_at || '').slice(0, 16).replace('T', ' ')}</span></div>
                    <p className="text-sm text-inkB dark:text-inkB-dark">{c.body}</p>
                  </div>
                ))}
              </div>
            ...
            {/* Activity */}
            <div>
              <div className="text-[10px] uppercase text-inkB dark:text-inkB-dark font-semibold mb-1.5 flex items-center gap-1"><ActivityIcon className="w-3.5 h-3.5" />Activity</div>
              <div className="space-y-1.5">
                {(data.activity || []).slice(0, 20).map(a => (
                  <div key={a.id} className="flex items-start gap-2 text-[11px] text-inkB dark:text-inkB-dark">
                    <span className="w-1.5 h-1.5 rounded-full bg-panel2 dark:bg-panel2-dark mt-1.5" />
                    <span><span className="text-inkB dark:text-inkB-dark font-medium">{a.actor_name || 'System'}</span> {a.action.replace(/_/g, ' ')}{a.detail ? ` · ${a.detail}` : ''} <span className="text-gray-400 dark:text-slate-500">{(a.created_at || '').slice(5, 16).replace('T', ' ')}</span></span>
                  </div>
                ))}
              </div>
            </div>
