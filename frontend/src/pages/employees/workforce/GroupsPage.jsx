    ...
    <div className="p-4 sm:p-6">
      ...
      {loading ? <Loading /> : groups.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {groups.map(g => (
            <button key={g.id} onClick={() => openDetail(g)} className="text-left rounded-2xl border border-slate-800 bg-slate-900 hover:border-slate-700 p-4">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold" style={{ background: g.color || '#8b5cf6' }}>{g.name?.[0]?.toUpperCase()}</div>
                <div className="min-w-0 flex-1"><div className="text-sm font-bold text-white truncate">{g.name}</div>
                  <div className="text-[11px] text-slate-500">{g.member_count || 0} members</div></div>
              </div>
        ...
        <Modal isOpen title="New Group" onClose={() => setShowCreate(false)} size="lg">
          <div className="p-5 space-y-3">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => setShowCreate(false)} className="flex-1 py-2 rounded-xl text-sm font-semibold text-gray-600 bg-gray-100">Cancel</button>
        ...
        <Modal isOpen title={detail.group?.name || 'Group'} onClose={() => setDetail(null)} size="lg">
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-gray-500">{detail.members?.length || 0} members</div>
              <div className="flex gap-2">
                <button onClick={() => { setAddingMembers(true); setNewMembers([]); }} className="inline-flex items-center gap-1 text-sm font-semibold text-brand-blue"><UserPlus className="w-4 h-4" />Add</button>
                <button onClick={() => setDeleteTarget(detail.group)} className="inline-flex items-center gap-1 text-sm font-semibold text-brand-red"><Trash2 className="w-4 h-4" />Delete</button>
              </div>
            </div>
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {(detail.members || []).length === 0 && <div className="text-sm text-gray-400 text-center py-6">No members yet.</div>}
              {(detail.members || []).map(m => (
                <div key={m.id} className="flex items-center gap-2.5 p-2 rounded-xl bg-gray-50">
                  <Avatar name={m.name} src={m.avatar} size="sm" />
                  <div className="min-w-0 flex-1"><div className="text-sm font-medium text-gray-900 truncate">{m.name}</div>
                    <div className="text-[11px] text-gray-400 truncate">{m.job_title || '—'}</div></div>
                  <button onClick={() => setTransfer({ memberIds: [m.id], toGroupId: '' })} className="p-1 text-gray-400 hover:text-brand-blue" title="Transfer"><ArrowRightLeft className="w-4 h-4" /></button>
                  <button onClick={() => removeMember(m.id)} className="p-1 text-gray-400 hover:text-red-500"><X className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
      ...
      {addingMembers && (
        <Modal isOpen title="Add Members" onClose={() => setAddingMembers(false)} size="md">
          <div className="p-5 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setAddingMembers(false)} className="flex-1 py-2 rounded-xl text-sm font-semibold text-gray-600 bg-gray-100">Cancel</button>
      ...
      {transfer && (
        <Modal isOpen title="Transfer Member" onClose={() => setTransfer(null)} size="sm">
          <div className="p-5 space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setTransfer(null)} className="flex-1 py-2 rounded-xl text-sm font-semibold text-gray-600 bg-gray-100">Cancel</button>
