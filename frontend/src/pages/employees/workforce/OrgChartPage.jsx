    ...
    <div role="button" tabIndex={0} onClick={() => onSelect(node)}
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-bold text-inkA dark:text-inkA-dark truncate">{node.name}</div>
          <div className="text-[11px] text-inkB dark:text-inkB-dark truncate">{node.job_title || '—'}</div>
        </div>
      </div>
      <div className="mt-2 flex items-center gap-1.5 flex-wrap">
        {node.department && (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-inkB dark:text-inkB-dark bg-panel2 dark:bg-panel2-dark px-1.5 py-0.5 rounded-full">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: node.department_color || '#64748b' }} />{node.department}
          </span>
        )}
        {node.status && <StatusBadge status={node.status} />}
      </div>
      <div className="mt-2 flex items-center gap-3 text-[10px] text-inkB dark:text-inkB-dark">
...
function IconBtn({ title, onClick, danger, children }) {
  return (
    <button title={title} onClick={onClick}
      className={`p-1.5 rounded-lg transition-colors ${danger ? 'text-inkB dark:text-inkB-dark hover:text-red-400 hover:bg-red-500/10' : 'text-inkB dark:text-inkB-dark hover:text-inkA dark:hover:text-white hover:bg-panel2 dark:hover:bg-panel2-dark'}`}>
      {children}
    </button>
  );
}
  ...
  return (
    <div className="p-4 sm:p-6 h-full flex-col">
      <PageHeader icon={GitBranch} title="Organization Chart" subtitle="Drag to pan · scroll to zoom · click a node to manage">
        ...
        <button onClick={() => setZoom(z => Math.max(0.4, z - 0.15))} className="p-2 rounded-xl bg-panel2 dark:bg-panel2-dark text-inkB dark:text-inkB-dark hover:text-inkA dark:hover:text-white"><ZoomOut className="w-4 h-4" /></button>
        <button onClick={resetView} className="p-2 rounded-xl bg-panel2 dark:bg-panel2-dark text-inkB dark:text-inkB-dark hover:text-inkA dark:hover:text-white" title="Reset view"><Maximize2 className="w-4 h-4" /></button>
      </PageHeader>
          ...
          {detail && (
            <div className="absolute top-3 right-3 w-64 rounded-2xl border-edge dark:border-edge-dark bg-slate-900/95 backdrop-blur p-4 shadow-xl">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0"><div className="text-sm font-bold text-inkA dark:text-inkA-dark truncate">{detail.employee?.name}</div>
                  <div className="text-[11px] text-inkB dark:text-inkB-dark truncate">{detail.employee?.job_title || '—'}</div></div>
              </div>
              {detail.managers?.length > 0 && (
                <div className="mt-3"><div className="text-[10px] uppercase text-inkB dark:text-inkB-dark font-semibold mb-1">Reports to</div>
  ...
  return (
    <Modal isOpen title={`Change manager — ${node.name}`} onClose={onClose} size="md">
      <div className="p-5 space-y-3">
        ...
        <p className="text-xs text-gray-400 dark:text-slate-500">Circular reporting is prevented automatically.</p>
...
function AddToTeamModal({ node, teams, onClose, onDone }) {
  ...
  return (
    <Modal isOpen title={`Add ${node.name} to a team`} onClose={onClose} size="md">
      <div className="p-5 space-y-3">
        {teams.length === 0 ? <p className="text-sm text-gray-400 dark:text-slate-500">No teams yet — use “Create team” first.</p> : (
