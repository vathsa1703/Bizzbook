    ...
    <div className="p-4 sm:p-6">
      <PageHeader icon={ClipboardList} title="Tasks" subtitle={`${tasks.length} task${tasks.length !== 1 ? 's' : ''}`}>
        <div className="relative">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search…" className="pl-8 pr-3 py-2 text-sm bg-panel2 dark:bg-panel2-dark border border-edge dark:border-edge-dark rounded-xl text-inkA dark:text-inkA-dark placeholder-gray-400 dark:placeholder-slate-500 outline-none focus:border-blue-500 w-36" />
        </div>
        <button onClick={() => setShowCreate(true)} className={btnPrimary}><Plus className="w-4 h-4" />New Task</button>
      </PageHeader>

      <div className="flex gap-2 mb-4 flex-wrap">
        <select value={status} onChange={e => setStatus(e.target.value)} className="px-3 py-1.5 text-sm bg-panel2 dark:bg-panel2-dark border border-edge dark:border-edge-dark rounded-xl text-inkA dark:text-inkA-dark outline-none">
