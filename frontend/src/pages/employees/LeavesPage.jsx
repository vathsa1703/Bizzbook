    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Track and manage employee time off</p>
        </div>
      ...
      <div className="flex gap-3">
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          className="bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:border-blue-500 min-w-[150px]">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-500 bg-slate-900 border border-slate-800 rounded-2xl">
          ...
          <p className="font-medium text-white">No leave records found</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {leaves.map(leave => (
            <div key={leave.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-4 hover:border-slate-700 transition-colors">
              <div className="flex-1 flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center font-bold text-white uppercase flex-shrink-0">
                  {leave.employee_name?.[0] || '?'}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-white truncate">{leave.employee_name}</h3>
                    <span className="text-xs text-slate-400">{leave.leave_type_name}</span>
                  </div>
                  <p className="text-sm text-slate-300 mt-1">{leave.start_date} to {leave.end_date} <span className="text-slate-500 font-medium">({leave.total_days} days)</span></p>
                  {leave.reason && <p className="text-xs text-slate-500 mt-1 italic truncate">"{leave.reason}"</p>}
                </div>
              </div>

              <div className="flex items-center justify-between md:justify-end gap-4 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t border-slate-800 md:border-0">
                ...
                {leave.status === 'pending' && (
                  <div className="flex gap-2">
                    {isAdmin && (
                      <>
                        <button onClick={() => { setTargetLeave(leave); setActionType('approve'); }} className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-lg text-xs font-semibold transition-colors">Approve</button>
                        <button onClick={() => { setTargetLeave(leave); setActionType('reject'); }} className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-semibold transition-colors">Reject</button>
                      </>
                    )}
                    {(!isAdmin || leave.employee_id === user?.employeeId) && (
                      <button onClick={() => { setTargetLeave(leave); setActionType('cancel'); }} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-semibold transition-colors">Cancel</button>
                    )}
                  </div>
                )}
              </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Apply Leave">
        <div className="space-y-4">
          {isAdmin && (
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Employee (Admin Override)</label>
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:border-blue-500 h-24 resize-none"
        ...
        message={
          actionType === 'reject' ? (
            <div className="mt-2 space-y-3 text-left">
              <p className="text-slate-300">Are you sure you want to reject this leave request from {targetLeave?.employee_name}?</p>
              <textarea 
                value={rejectReason} onChange={e => setRejectReason(e.target.value)}
                placeholder="Optional reason for rejection..."
                className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm outline-none focus:border-red-500 h-20 resize-none"
              />
            </div>
