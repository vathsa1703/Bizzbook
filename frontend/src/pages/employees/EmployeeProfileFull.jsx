        ...
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 mt-3">
              ...
              <span className="flex items-center gap-1.5 text-sm text-inkB dark:text-inkB-dark"><Calendar className="w-4 h-4" /> Joined: {employee.joining_date}</span>
            </div>
          </div>
        </div>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all
                ${activeTab === t.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'bg-panel2 dark:bg-panel2-dark text-inkB dark:text-inkB-dark hover:bg-panel2 dark:hover:bg-panel2-dark hover:text-inkA dark:hover:text-white'}
              `}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-panel dark:bg-panel-dark border border-edge dark:border-edge-dark rounded-2xl p-5">
              ...
              <div className="space-y-4">
                ...
                <div><div className="text-xs text-inkB dark:text-inkB-dark mb-1">Employment Type</div><div className="text-sm font-medium text-inkA dark:text-inkA-dark">{employee.employment_type || 'Full Time'}</div></div>
                <div><div className="text-xs text-inkB dark:text-inkB-dark mb-1">Branch</div><div className="text-sm font-medium text-inkA dark:text-inkA-dark">{employee.branch_name || 'Headquarters'}</div></div>
                <div><div className="text-xs text-inkB dark:text-inkB-dark mb-1">Basic Salary</div><div className="text-sm font-medium text-inkA dark:text-inkA-dark">₹{employee.salary?.toLocaleString() || 0} / month</div></div>
              </div>
            </div>
        ...
        {activeTab === 'documents' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-bold text-inkA dark:text-inkA-dark">Employee Documents</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div key={doc.id} className="bg-panel dark:bg-panel-dark border border-edge dark:border-edge-dark rounded-2xl p-4 flex items-center justify-between group">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-inkA dark:text-inkA-dark truncate">{doc.doc_name}</div>
                      <div className="text-[10px] text-inkB dark:text-inkB-dark">{(doc.file_size / 1024).toFixed(1)} KB • {new Date(doc.created_at).toLocaleDateString()}</div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <a href={api.getDocumentUrl(doc.id)} target="_blank" rel="noreferrer" className="p-2 text-inkB dark:text-inkB-dark hover:text-blue-400 bg-panel2 dark:bg-panel2-dark hover:bg-blue-500/10 rounded-lg transition-colors">
                      <Download className="w-4 h-4" />
                    </a>
                    <button onClick={() => handleDeleteDoc(doc.id)} className="p-2 text-inkB dark:text-inkB-dark hover:text-red-400 bg-panel2 dark:bg-panel2-dark hover:bg-red-500/10 rounded-lg transition-colors">
              ...
              {documents.length === 0 && (
                <div className="col-span-full py-10 text-center text-inkB dark:text-inkB-dark">No documents uploaded yet.</div>
              )}
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-panel dark:bg-panel-dark border border-edge dark:border-edge-dark rounded-2xl overflow-hidden flex flex-col">
              <div className="p-5 border-b border-edge dark:border-edge-dark flex justify-between items-center bg-panel dark:bg-panel-dark">
                ...
                {!editSalaryMode ? (
                  <button onClick={() => setEditSalaryMode(true)} className="text-blue-600 dark:text-blue-400 hover:text-blue-300 text-sm font-semibold flex items-center gap-1">
                    <Edit2 className="w-4 h-4" /> Edit
                  </button>
                ) : (
                  <div className="flex gap-2">
                    <button onClick={() => setEditSalaryMode(false)} className="text-inkB dark:text-inkB-dark hover:text-inkA dark:hover:text-white text-sm font-semibold">Cancel</button>
                    <button onClick={saveSalaryStructure} className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-300 text-sm font-semibold">Save</button>
                  </div>
                )}
              </div>
              <div className="p-5 flex-1 overflow-y-auto">
                {editSalaryMode ? (
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="text-xs font-bold text-inkB dark:text-inkB-dark uppercase tracking-wider mb-3">Earnings</div>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">Basic</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.basic}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">HRA</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.hra}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">DA</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.da}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">Medical</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.medical}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">Travel</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.travel}</span></div>
                      </div>
                      <div className="mt-3 pt-3 border-t border-edge dark:border-edge-dark flex justify-between font-bold">
                        <span className="text-blue-600 dark:text-blue-400">Gross Salary</span><span className="text-blue-600 dark:text-blue-400">₹{salaryStructure.gross_salary}</span>
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-bold text-inkB dark:text-inkB-dark uppercase tracking-wider mb-3">Deductions</div>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">PF (Employee)</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.pf_employee}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">ESI (Employee)</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.esi_employee}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">TDS</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.tds}</span></div>
                        <div className="flex justify-between"><span className="text-inkB dark:text-inkB-dark">Professional Tax</span><span className="text-inkA dark:text-inkA-dark font-medium">₹{salaryStructure.professional_tax}</span></div>
                      </div>
                      <div className="mt-3 pt-3 border-t border-edge dark:border-edge-dark flex justify-between font-bold">
                  ...
                  <div className="text-center py-10 text-inkB dark:text-inkB-dark">No salary structure defined yet.</div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {salarySlips.map(slip => (
                  <div key={slip.id} className="p-4 rounded-xl border border-edge dark:border-edge-dark bg-panel2/40 dark:bg-panel2-dark/40 flex items-center justify-between">
                    ...
                    <button className="px-3 py-1.5 bg-panel2 dark:bg-panel2-dark hover:bg-panel2 dark:hover:bg-panel2-dark text-inkA dark:text-inkA-dark text-xs font-semibold rounded-lg transition-colors">
                      View Slip
                    </button>
                  </div>
                ))}
                {salarySlips.length === 0 && (
                  <div className="text-center py-8 text-inkB dark:text-inkB-dark">No payroll history found.</div>
                )}
              </div>
            </div>
          </div>
