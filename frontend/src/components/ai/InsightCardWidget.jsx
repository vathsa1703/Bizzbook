...
export default function InsightCardWidget() {
  ...
  if (loading) {
    return (
      <div className="mb-6 h-32 flex items-center justify-center text-sm text-gray-400 dark:text-slate-500 border border-dashed rounded-xl border-edge dark:border-edge-dark bg-panel2/50 dark:bg-panel2-dark/50">
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-3 px-1">
        <h3 className="text-sm font-bold text-inkA dark:text-inkA-dark flex items-center gap-1.5">
          <Sparkles size={16} className="text-violet-600 dark:text-violet-400" />
          AI Advisor
          {note && <span className="text-[10px] font-normal text-inkB dark:text-inkB-dark bg-panel2 dark:bg-panel2-dark px-2 py-0.5 rounded-full ml-2">{note}</span>}
        </h3>
      </div>
