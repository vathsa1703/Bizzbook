    ...
    <Modal isOpen title={editing ? 'Edit Guideline' : 'New Trade Guideline'} onClose={onCancel} size="lg">
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 uppercase">Code</label><input className={inp} value={f.code} disabled={editing} onChange={e => set('code', e.target.value)} placeholder="TRADE_NEW_RULE" /></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 uppercase">Category</label><select className={inp} value={f.category} onChange={e => set('category', e.target.value)}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
        ...
        <div><label className="text-[10px] font-bold text-gray-400 uppercase">Renewal Interval (months)</label><input type="number" className={inp} value={f.renewal_interval_months || ''} onChange={e => set('renewal_interval_months', e.target.value)} /></div>
        <div><label className="text-[10px] font-bold text-gray-400 uppercase">Department / Authority</label><input className={inp} value={f.department || ''} onChange={e => set('department', e.target.value)} /></div>
        <div><label className="text-[10px] font-bold text-gray-400 uppercase">Official Website</label><input className={inp} value={f.official_website || ''} onChange={e => set('official_website', e.target.value)} /></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 uppercase">Fees</label><input className={inp} value={f.fees || ''} onChange={e => set('fees', e.target.value)} /></div>
          <div><label className="text-[10px] font-bold text-gray-400 uppercase">Processing Time</label><input className={inp} value={f.processing_time || ''} onChange={e => set('processing_time', e.target.value)} /></div>
        </div>
        <div><label className="text-[10px] font-bold text-gray-400 uppercase">Renewal Requirement</label><input className={inp} value={f.renewal_requirement || ''} onChange={e => set('renewal_requirement', e.target.value)} /></div>
        ...
        <div><label className="text-[10px] font-bold text-gray-400 uppercase">AI Explanation</label><textarea className={inp} rows={2} value={f.ai_explanation || ''} onChange={e => set('ai_explanation', e.target.value)} /></div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1"><label className="text-[10px] font-bold text-gray-400 uppercase">Applies When (all true)</label>
            <button onClick={() => set('conditions', [...f.conditions, { attribute: 'export_enabled', operator: 'is_true', value: '' }])} className="text-[11px] font-bold text-blue-600">+ Condition</button></div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1"><label className="text-[10px] font-bold text-gray-400 uppercase">Required Documents</label>
            <button onClick={() => set('documents', [...f.documents, ''])} className="text-[11px] font-bold text-blue-600">+ Document</button></div>
          {f.documents.map((d, i) => (
            <div key={i} className="flex items-center gap-1 mb-1">
...
export default function AdminGuidelines() {
  ...
  if (forbidden) return <p className="text-sm text-gray-400 text-center py-10">Admin access required to manage trade guidelines.</p>;
  ...
  return (
    <div>
      ...
      <p className="text-[11px] text-gray-400 mb-2">{filtered.length} guidelines</p>
          ...
          <div key={r.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-gray-400 mt-0.5">{r.code} · {r.category} · {r.frequency} · {(r.conditions || []).length} conditions</p>
              </div>
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${r.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>{r.is_active ? 'Active' : 'Off'}</span>
            </div>
            <div className="flex items-center gap-1 mt-2 pt-2 border-t border-gray-50">
              ...
              <button onClick={() => del(r)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 ml-auto" title="Delete"><Trash2 size={15} /></button>
            </div>
          </div>
        ))}
      </div>
