    ...
    <Modal isOpen title={editing ? 'Edit Rule' : 'New Compliance Rule'} onClose={onCancel} size="lg">
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Code</label><input className={inp} value={f.code} disabled={editing} onChange={e => set('code', e.target.value)} placeholder="IN_NEW_RULE" /></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Category</label><select className={inp} value={f.category_key} onChange={e => set('category_key', e.target.value)}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Due Day</label><input type="number" className={inp} value={f.due_day || ''} onChange={e => set('due_day', e.target.value)} /></div>
          <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Due Month</label><input type="number" className={inp} value={f.due_month || ''} onChange={e => set('due_month', e.target.value)} /></div>
          <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Renew (mo)</label><input type="number" className={inp} value={f.renewal_interval_months || ''} onChange={e => set('renewal_interval_months', e.target.value)} /></div>
        </div>
        <div className="grid grid-cols-3 gap-3">
        ...
        <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Department</label><input className={inp} value={f.department || ''} onChange={e => set('department', e.target.value)} /></div>
        <div><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Portal URL</label><input className={inp} value={f.portal_url || ''} onChange={e => set('portal_url', e.target.value)} /></div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1"><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Applies When (all true)</label>
            <button onClick={() => set('conditions', [...f.conditions, { attribute: 'gst_registered', operator: 'is_true', value: '' }])} className="text-[11px] font-bold text-accent">+ Condition</button></div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1"><label className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Required Documents</label>
            <button onClick={() => set('documents', [...f.documents, ''])} className="text-[11px] font-bold text-accent">+ Document</button></div>
          {f.documents.map((d, i) => (
            <div key={i} className="flex items-center gap-1 mb-1">
...
export default function AdminRules() {
  ...
  if (forbidden) return <p className="text-sm text-gray-400 dark:text-slate-500 text-center py-10">Admin access required to manage compliance rules.</p>;
  ...
  return (
    <div>
      ...
      <p className="text-[11px] text-gray-400 dark:text-slate-500 mb-2">{filtered.length} rules</p>
          ...
          <div key={r.id} className="bg-panel dark:bg-panel-dark border border-edge dark:border-edge-dark rounded-xl p-3 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">{r.code} · {r.category_key} · {r.frequency} · {(r.conditions || []).length} conditions</p>
              </div>
              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${r.is_active ? 'bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-panel2 dark:bg-panel2-dark text-gray-400 dark:text-slate-500'}`}>{r.is_active ? 'Active' : 'Off'}</span>
            </div>
            <div className="flex items-center gap-1 mt-2 pt-2 border-t border-edge dark:border-edge-dark">
              ...
              <button onClick={() => del(r)} className="p-1.5 rounded-lg hover:bg-brand-redSoft text-gray-400 dark:text-slate-500 hover:text-brand-red ml-auto" title="Delete"><Trash2 size={15} /></button>
            </div>
          </div>
        ))}
      </div>
