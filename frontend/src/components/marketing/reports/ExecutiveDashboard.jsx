  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard icon={TrendingUp} title="Total Revenue" value={`$${data.revenue.toLocaleString()}`} color="text-emerald-500" bg="bg-emerald-50" />
        <StatCard icon={Target} title="Campaigns" value={data.campaigns} color="text-blue-500" bg="bg-blue-50" />
        <StatCard icon={Users} title="Active Customers" value={data.activeCustomers} color="text-violet-500" bg="bg-violet-50" />
        <StatCard icon={Activity} title="New Customers" value={data.newCustomers} color="text-orange-500" bg="bg-orange-50" />
      </div>

      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <h3 className="text-sm font-bold text-gray-900 mb-6">Revenue Trend</h3>
        <div className="h-72">
          {data.revenueTrend && data.revenueTrend.length > 0 ? (
            ...
            <div className="flex h-full items-center justify-center text-sm text-gray-400">No revenue data for this period</div>
          )}
        </div>
      </div>
    </div>
