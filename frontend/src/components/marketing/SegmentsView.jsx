...
export default function SegmentsView() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-bold text-gray-900">Custom Segments</h3>
      {loading ? <Loader2 className="animate-spin" /> : (
        <div className="space-y-2">
          {segments.map(s => (
            <div key={s.id} className="p-3 bg-gray-50 border rounded-xl flex justify-between items-center">
              <div>
                <p className="font-bold text-sm text-gray-900">{s.name}</p>
                <p className="text-xs text-gray-500">{s.description || 'No description'}</p>
              </div>
              <span className="text-xs bg-white px-2 py-1 rounded shadow-sm border font-bold text-gray-600">ID: {s.id} | {s.logic_type}</span>
            </div>
          ))}
        </div>
      )}
      <form onSubmit={handleCreate} className="pt-4 border-t space-y-3">
        <h4 className="font-bold text-sm text-gray-900">Create New Segment</h4>
